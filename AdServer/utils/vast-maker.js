...
function VastBuilder(params) {
  let vastObject = {};
  let adList = [];
  params.adList.forEach(element => {
    if(element.ADS_STATUS !== 'COMPLETED'){
      adList.push({
        universalId: element.UNIVERSAL_ID,
        id: element.ID,
        url: [
          element.URL
        ],
  ...
  console.log(selectedAds,adsDuration)
...
function AttachStandAloneAds(vast4, ads, params, podSize) {
  ...
  for (let i = 0; i < ads.length; i++) {
    vast4
      ...
      .addAdSystem(`W3Ads Server`)
      .addAdTitle(`Ad That W3Ads-Server Wants Player To See #${i + podSize}`)
      .attachCreatives()
      .attachCreative({
      ...
      .and()
      .attachVideoClicks()
      .addClickThrough("https://github.com/Harsh8196/w3Ads", { id: "W3Ads Server" })
      .and()
...
function AttachPodAds(vast4, podAds, params) {
  for (let i = 0; i < podAds.length; i++) {
    mediaNode = vast4
      ...
      .attachInLine()
      .addImpression(`http://${params.adserverHostname}/api/v1/sessions/${params.sessionId}/tracking?adId=${podAds[i].id}&progress=vast`, { id: `IMPRESSION-ID_00${i + 1}` })
      .addAdSystem(`W3Ads Server`)
      .addAdTitle(`Ad That W3Ads-Server Wants Player To See #${i + 1}`)
      .attachCreatives()
      .attachCreative({
      ...
      .attachLinear()
      .attachTrackingEvents()
      .addTracking(`http://${params.adserverHostname}/api/v1/sessions/${params.sessionId}/tracking?adId=${podAds[i].id}&progress=0`, { event: "start" })
      .addTracking(`http://${params.adserverHostname}/api/v1/sessions/${params.sessionId}/tracking?adId=${podAds[i].id}&progress=25`, { event: "firstQuartile" })
      .addTracking(`http://${params.adserverHostname}/api/v1/sessions/${params.sessionId}/tracking?adId=${podAds[i].id}&progress=50`, { event: "midpoint" })
      .addTracking(`http://${params.adserverHostname}/api/v1/sessions/${params.sessionId}/tracking?adId=${podAds[i].id}&progress=75`, { event: "thirdQuartile" })
      .addTracking(`http://${params.adserverHostname}/api/v1/sessions/${params.sessionId}/tracking?adId=${podAds[i].id}&progress=100`, { event: "complete" })
      .and()
      .attachVideoClicks()
      .addClickTracking(`http://${params.adserverHostname}/api/v1/sessions/${params.sessionId}/tracking?adId=${podAds[i].id}&progress=click`, { event: "click" })
      .addClickThrough("https://github.com/Harsh8196/w3Ads", { id: "W3Ads Server" })
      .and()
